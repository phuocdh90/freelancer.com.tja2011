...
public class DataGenerator {
	...
	private void read_xCyD_Data(FileIO dataFile) {
		...
		for (int i = 0; i < Parameters.dataSize; i++) {
			...
			data[i] = new Datum(dataDetails, 1, 1, 1, 1, 1, 1, 1); // second to
																	// eighth
																	// parameter
																	// is just a
																	// dummy
	...
	/**
	 * public void createBackup() { for (int i =0; i < data.length; i++) {
	 * backupData[i] = new Datum(data[i].getValue(), data[i].getSymbol());
	 * Parameters.setDistanceMeasure(backupData[i]); } }
	 */

	/**
	 * public void randomiseDataPosn() { int randXPos, randYPos;
	 * 
	 * for (int i =0; i < data.length; i++) { randXPos =
	 * (int)Math.floor(Math.random()*Parameters.maxX); randYPos =
	 * (int)Math.floor(Math.random()*Parameters.maxY);
	 * this.data[i].setPos(randXPos, randYPos); createBackup(); } }
	 ***/
	...
	private void readFileData(FileIO dataFile) {
		...
		for (int i = 0; i < Parameters.dataSize; i++) {
			dataDetails = dataFile.readString();
			System.out.println("dataDetails before add:" + dataDetails);

			// int spaceCount = 0;
			// for (char c : dataDetails.toCharArray()) {
			// if (c == '\t') {
			// spaceCount++;
			// }
			// }

			String[] st = dataDetails.split("\\s");
			Map<Integer, Double> st1 = new HashMap<Integer, Double>();
			Map<Integer, Double> st2 = new HashMap<Integer, Double>();
			Map<Integer, Double> st3 = new HashMap<Integer, Double>();
			st1.put(0, Double.parseDouble(st[0]));
			st2.put(0, Double.parseDouble(st[0]));
			st3.put(0, Double.parseDouble(st[0]));

			double value = 0.0;
			String datas = "";

			for (int j = 1; j < 5; j++) {
				st1.put(j, Double.parseDouble("0"));
				st2.put(j, Double.parseDouble("0"));
				st3.put(j, Double.parseDouble("0"));
			}
			...
			if (selection1 == 2) {
				for (int p = 1; p < st.length; p++) {
					...
					if (!st[p].equals("")) {
						st1.put(p, Double.parseDouble(st[p]));
						st3.put(p, Double.parseDouble(st[p]));
			...
			data[i] = new Datum(datas, 1.0, 'a');

		}
	}

	// UNIFORM distribution of data from startRange to endRange
	public void generateSytheticData(int startRange, int endRange,
			int startIndex, int endIndex, char symbol) {
	...
	private void readWineData(FileIO dataFile) {
		...
		for (int i = 0; i < Parameters.dataSize; i++) {
			dataDetails = dataFile.readString();
			data[i] = new Datum(dataDetails, 1); // second parameter is just a
													// dummy
		}
	...
	private void readWisconsinData(FileIO dataFile) {
		...
		for (int i = 0; i < Parameters.dataSize; i++) {
			dataDetails = dataFile.readString();
			data[i] = new Datum(dataDetails, 1, 1); // second and thrid
													// parameter is just a dummy
		}
	...
	public void printMatrix() {
		...
		for (z = 1; z <= N; z++)
			System.out.print(fourDigits.format(z));
		System.out.println("");

		for (i = 1; i <= N; i++) {
			for (j = 0; j < i; j++) {
				System.out.print((i - 1) + ":" + j + "="
						+ twoDigits.format(matrix[i - 1][j]) + ", ");
			}
			System.out.println("");
		}
	}
