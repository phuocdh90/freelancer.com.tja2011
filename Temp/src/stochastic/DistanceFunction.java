...
class Euclidean implements DistanceFunction {
	...
	public double precomputed(Datum d) {
		...
		double dist = 0;
		if (index1 == index2) {
			// System.out.println("Same same hor, problem lah! index: " + index1
			// + " and " + myDatum + " and " + d);
			return 0.0;
		}
		if (index1 > index2) {
	...
	public double compute(Datum d) {
		...
		if (i > j) {
			return Parameters.matrix[i - 1][j]; // must be used in conjuction
												// with Julia neighbourhood
												// function
		} else {
...
class Cosine implements DistanceFunction {
	...
	public double precompute(Datum d) {
		...
		cosineSim = sum_ikjk / (Math.sqrt(sum_ikik * sum_jkjk)); // cosine
																	// similarity
																	// metric

		JuliaDistance = (1.0 - 0.5 * (1.0 + cosineSim)); // Julia's method of
															// converting to
															// Dis-similarity
		// JuliaSimilarity = 1 - JuliaDistance/Parameters.alpha;

		return JuliaDistance; // must be used in conjuction with Julia
...
class General2DEucliden implements DistanceFunction {
	...
	public double precompute(Datum d) {
		...
		double dist = Parameters.euclidean(dVal[0], dVal[1],
				this.myDatum.value[0], this.myDatum.value[1]);
		return dist;
	}
