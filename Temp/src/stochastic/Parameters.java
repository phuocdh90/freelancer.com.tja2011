...
class iNode {
	...
	iNode(double max[], double min[], int ml) {
		...
		meanVector = new double[min.length];
		for (int i = 0; i < max.length; i++) {
			meanVector[i] = 0;
		}
		...
		if (myLevel == maxHeight) {
			...
			/*
			 * ArrayList randomList = new ArrayList(); for(int i=0;
			 * i<max.length; i++) { randomList.add(i); } int idx = (int)
			 * Parameters.generator.nextInt(randomList.size()); dimIndex =
			 * ((Integer)randomList.get(idx)).intValue(); while((min[dimIndex]
			 * == max[dimIndex])&&(randomList.size() > 0)){ if
			 * (randomList.size()==1){ dimIndex =
			 * ((Integer)randomList.get(0)).intValue(); randomList.remove(0);
			 * }else { randomList.remove(idx); idx = (int)
			 * Parameters.generator.nextInt(randomList.size()); dimIndex =
			 * ((Integer)randomList.get(idx)).intValue(); } }
			 * if(randomList.size() == 0){ dimIndex = (int)
			 * Parameters.generator.nextInt(max.length); }
			 */

			// half split seems better
			// this.splitPoint = min[dimIndex] +
			// Math.random()*(max[dimIndex]-min[dimIndex]);
			this.splitPoint = (min[dimIndex] + max[dimIndex]) / 2;
			// System.out.println("my level " + ml +
			// " I am a node with split point = " + splitPoint + ", dimIndex = "
			// + dimIndex);
			// System.out.println("min[dimIndex], max[dimIndex]" + min[dimIndex]
			// +"," +max[dimIndex]);
			...
			/*
			 * double oldMax = max[dimIndex]; double oldMin = min[dimIndex];
			 * min[dimIndex] = splitPoint; leftChildNode = new iNode(max, min,
			 * ml+1);
			 * 
			 * min[dimIndex] = oldMin; max[dimIndex] = splitPoint;
	...
	public void updateMean(Datum d) {
		// if (Parameters.checkType(d) == 0) {
		for (int i = 0; i < d.dim; i++) {
			meanVector[i] = 0.4 * meanVector[i] + 0.6 * d.value[i];
		}
		// }
		if (!isLeaf) {
			if (d.value[dimIndex] > splitPoint) {
				leftChildNode.updateMean(d);
			} else {
				rightChildNode.updateMean(d);
			}
		}
	}

	public double[] getMean(Datum d) {
		// if (Parameters.checkType(d) == 0) {

		// }
		if (!isLeaf) {
			if (d.value[dimIndex] > splitPoint) {
				return leftChildNode.getMean(d);
			} else {
				return rightChildNode.getMean(d);
			}
		} else {
			return meanVector;
		}
	...
	public void printMass1Mass2() {
		...
		if (this.myLevel == 10) {
			if (mySize != 0 || mySize2 != 0)
				System.out.println("mySize:," + mySize + ", mySize2:,"
						+ mySize2);
		} else {
	...
	public void printAugmentedMass1Mass2_Ver1() {
		if (isLeaf) {
			...
			if (mySize2 == 0)
				mySize2 = 1;
			// System.out.println("my level:, " + myLevel + ", mySizePathLen:,"
			// + (Math.log(mySize) + (double)myLevel) + ", mySize2_PathLen:," +
			// (Math.log(mySize2) + (double)myLevel));
			System.out.println("my level:, " + myLevel + ", mySizePathLen:,"
					+ (log2(mySize) + (double) myLevel) + ", mySize2_PathLen:,"
					+ (log2(mySize2) + (double) myLevel));
		} else {
			// if ((leftChildNode.mySize > 40)||(leftChildNode.mySize2 > 40)) {
			if (leftChildNode.mySize > 20) {
				leftChildNode.printAugmentedMass1Mass2_Ver1();
			} else {
				if (leftChildNode.mySize == 0)
					leftChildNode.mySize = 1;
				if (leftChildNode.mySize2 == 0)
					leftChildNode.mySize2 = 1;
				System.out
						.println("my level:, "
								+ leftChildNode.myLevel
								+ ", mySizePathLen:,"
								+ (log2(leftChildNode.mySize) + (double) leftChildNode.myLevel)
								+ ", mySize2_PathLen:,"
								+ (log2(leftChildNode.mySize2) + (double) leftChildNode.myLevel));
			}
			...
			if (rightChildNode.mySize > 20) {
				...
				if (rightChildNode.mySize == 0)
					rightChildNode.mySize = 1;
				if (rightChildNode.mySize2 == 0)
					rightChildNode.mySize2 = 1;
				System.out
						.println("my level:, "
								+ rightChildNode.myLevel
								+ ", mySizePathLen:,"
								+ (log2(rightChildNode.mySize) + (double) rightChildNode.myLevel)
								+ ", mySize2_PathLen:,"
								+ (log2(rightChildNode.mySize2) + (double) rightChildNode.myLevel));
			}
		}
	}
	...
	public ArrayList getAugmentedMass1Mass2_Ver1(ArrayList alist1) {
		...
		if (isLeaf) {
			...
			if (leftChildNode.mySize > 10) {
				...
				if (leftChildNode.mySize2 == 0)
					leftChildNode.mySize2 = 1;
				// System.out.println("my level:, " + leftChildNode.myLevel +
				// ", mySizePathLen:," + (log2(leftChildNode.mySize) +
				// (double)leftChildNode.myLevel) + ", mySize2_PathLen:," +
				// (log2(leftChildNode.mySize2) +
				// (double)leftChildNode.myLevel));
				double val2[] = new double[2];
			...
			if (rightChildNode.mySize > 10) {
				...
				if (rightChildNode.mySize2 == 0)
					rightChildNode.mySize2 = 1;
				// System.out.println("my level:, " + rightChildNode.myLevel +
				// ", mySizePathLen:," + (log2(rightChildNode.mySize) +
				// (double)rightChildNode.myLevel) + ", mySize2_PathLen:," +
				// (log2(rightChildNode.mySize2) +
				// (double)rightChildNode.myLevel));
				double val2[] = new double[2];
	...
	public ArrayList getAugmentedMass1Mass2_Ver4(ArrayList alist1) {
		...
		if (isLeaf) {
			...
			if (leftChildNode.mySize > 20) {
				...
				double val2[] = new double[2];
				val2[0] = tempMySize
						* Math.pow(2, (double) leftChildNode.myLevel);
				val2[1] = tempMySize2
						* Math.pow(2, (double) leftChildNode.myLevel);
			...
			if (rightChildNode.mySize > 20) {
				...
				double val2[] = new double[2];
				val2[0] = tempMySize
						* Math.pow(2, (double) rightChildNode.myLevel);
				val2[1] = tempMySize2
						* Math.pow(2, (double) rightChildNode.myLevel);
				alist1.add(val2);
			}
	...
	public void printAugmentedMass1Mass2() {
		if (marked) {
			...
			if (mySize2 == 0)
				mySize2 = 1;
			System.out.println("my level:, " + myLevel + ", mySizePathLen:,"
					+ (Math.log(mySize) + (double) myLevel)
					+ ", mySize2_PathLen:,"
					+ (Math.log(mySize2) + (double) myLevel));
		}
		if (!isLeaf) {
	...
	public double computeMassPathLen1(Datum d) {
		...
		// this.distScore = Math.log(this.mySize)/Math.log(2) +
		// (double)this.myLevel;
...
class jNode {
	...
	jNode(double max[], double min[], int ml) {
		...
		// System.out.println("min[dimIndex] max[dimIndex] " + min[dimIndex] +
		// "," + max[dimIndex]);
		System.out.println("I am a node with split point = " + splitPoint
				+ ", dimIndex = " + dimIndex);
		myMax = new double[max.length];
		myMin = new double[min.length];
	...
	public void assign(Datum d) {
		size++;
		if (isLeaf) {
			// System.out.println("This is a leaf with " + leafCounter +
			// " items");
			// myData[leafCounter]=d;
			myData.add(d);
			leafCounter++;
			if ((leafCounter >= Parameters.maxLeafCount)
					&& (myLevel < Parameters.maxExpansionLevel)) {
	...
	public double score(Datum d) {
		if (isLeaf) {
			...
			// System.out.println("LeafCounter = " + leafCounter + ", My level "
			// + this.myLevel);
			// System.out.println("path length score = " + ((double)this.myLevel
			// + 2*(Math.log((double)leafCounter-1.0)+0.5772156649) -
			// (2*((double)leafCounter-1.0)/((double)leafCounter))));

			if (leafCounter < 2) {
				return (double) this.myLevel;
			} else {
				return ((double) this.myLevel + 2
						* (Math.log((double) leafCounter - 1.0) + 0.5772156649) - (2 * ((double) leafCounter - 1.0) / ((double) leafCounter)));
			}
...
public class Parameters {
	...
	public static int dataSet = 4; // 1 for Wine, 2 for Iris, 3 for Square, 4
									// for Wisconsin
	...
	public static int numClusters = -1; // this value must be initilaised in app
										// initializer
	...
	public static double computeAUROC(ArrayList anomalyTypeArray,
			ArrayList anormalyScoresArray) {
		...
		for (int i = 0; i < adnormalScores.size(); i++) {
			...
			for (int j = 0; j < normalScores.size(); j++) {
				if (adnormalScores.get(i) < normalScores.get(j)) {
					// System.out.println("adnormal score = " +
					// adnormalScores.get(i));
	...
	public static int checkType(Datum d) {
		...
		if ((Parameters.dataSetName.compareTo("http") == 0)
				...
				|| (Parameters.dataSetName.compareTo("Wisconsin") == 0) || // Wisconsin:
																			// 1
																			// is
																			// anomaly,
																			// 2
																			// is
																			// normal
				(Parameters.dataSetName.compareTo("Pima") == 0) || // pima: 1 is
																	// anomaly,
																	// 2 is
																	// normal
				(Parameters.dataSetName.compareTo("ionosphere") == 0)) {
			if (d.symbol != '+') { // use != '+' for
									// smtp/htt/pima/wisconsin/ionosphere/satellite/annthyroid
				anomalyType = 0;
			} else {
				anomalyType = 1;
			}
	...
	/**
	 * Modified from Julia's work Compute the eucclidean distance between two
	 * map positions A and B
	 * 
	 * @param i1
	 *            y coordinate for position A
	 * @param j1
	 *            x coordinate for position A
	 * @param i2
	 *            y coordinate for position B
	 * @param j2
	 *            x coordinate for position B
	...
	public static double generalCosine(double[] data1, double[] data2) {
		...
		// div = sum_ikjk/(Math.sqrt(sum_ikik) * Math.sqrt(sum_jkjk)); same as
		// below mathmthematically
	...
	public static double stDev(ArrayList var1, double maxDunn) {
		...
		for (int i = 0; i < var1.size(); i++) {
			...
			if (aDunn == maxDunn)
				continue; // exclude the peak Dunn itself
	...
	public static void showCriteria(boolean showFlag, int stage,
			ArrayList fmeasure, ArrayList Dunns, ArrayList Variances,
			ArrayList silhouette, ArrayList daviesDoublin, int bagsLength) {
		String result = "\n";

		if (showFlag) {
			result += "Stage:" + stage;
			result += ", oldDunn=,"
					+ to4DecPlace(Double.parseDouble(Dunns.get(stage)
							.toString()));
			result += ", FMeas=,"
					+ to4DecPlace(Double.parseDouble(fmeasure.get(stage)
							.toString()));
			result += ", Intra-Variance=,"
					+ to4DecPlace(Double.parseDouble(Variances.get(stage)
							.toString()));
			result += ", silhouetteCoefficient=,"
					+ to4DecPlace(Double.parseDouble(silhouette.get(stage)
							.toString()));
			result += ", DaviesDouldinIndex=,"
					+ to4DecPlace(Double.parseDouble(daviesDoublin.get(stage)
							.toString()));
	...
	public static void showArrayList(ArrayList alist) {
		for (int i = 0; i < (alist.size()); i++) {
			System.out.print(i + ":,"
					+ to4DecPlace(Double.parseDouble(alist.get(i).toString()))
					+ ", ");
			if ((i > 0) && ((i % 9) == 0))
				System.out.println();
		}
		...
		for (int i = 0; i < (alist.size() - 1); i++) {
			...
			if (denom > 0) {
				System.out.print((i + 1)
						...
						+ i
						+ ":"
						+ to4DecPlace(Double.parseDouble(alist.get(i + 1)
								.toString())
								/ Double.parseDouble(alist.get(i).toString()))
						+ ", ");
			}
			if (i == 10)
				System.out.println();
	...
	public static int findMaxStageWithinKrange(ArrayList grpAvgDistRatio) {
		...
		return maxStage + 1; // add 1 to compensate the lost of 1 stage in ratio
								// computation
	}

	public static double getTScoresWeight(ArrayList alist) {
		int bestK = scanFirstBigChange(alist);
		if (bestK == -1) { // no best K, just assign all zero weight ==> no
							// effect
			return 0;
		}
		return 1;
	...
	public static int scanFirstBigChange_BK(ArrayList alist) {
		...
		for (int i = 0; i < (alist.size()); i++) {
			...
			if ((tScore > threshold) && (!found)) { // was 100
				if (i < 18) {
					if ((Double.parseDouble(alist.get(i + 1).toString()) > 20)
							&& (Double.parseDouble(alist.get(i + 2).toString()) > 20)) {
						k = i - 1; // last one is the optimal
						found = true;
					} else {
						threshold = 100;
	...
	public static double getMinVarianceWeight(ArrayList alist) {
		...
		for (int i = beforeStages; i < minStage; i++) {
			aVar = Double.parseDouble(alist.get(i + 1).toString())
					- Double.parseDouble(alist.get(i).toString());
		...
		if ((minStage == Parameters.totalStage - 1) || (minStage == 0)) {
			...
			leftVariance = (Double.parseDouble(alist.get(minStage + 1)
					.toString()) - Double.parseDouble(alist.get(minStage)
					.toString()));
		}
	...
	public static ArrayList updateDiff(ArrayList alist) {
		...
		diffList.add(0.0);
		for (int i = 0; i < (alist.size() - 1); i++) {
			diff = Double.parseDouble(alist.get(i + 1).toString())
					- Double.parseDouble(alist.get(i).toString());
	...
	public static ArrayList updateTscore(ArrayList alist) {
		...
		if (initN < 0) {
			for (int i = 0; i < Math.abs(initN); i++) { // make the t scores
														// contains the same #
														// as total stage
				tScoreList.add(0);
			}
			...
			for (int i = 0; i < initN; i++) { // gather all previous experiences
				cumList.add(alist.get(i)); // at the same time update cummuative
											// list
				tScoreList.add(0);
			}
		} else {
		...
		for (int i = initN; i < alist.size(); i++) { // first 5 elements set
														// with zero t scores
			value = Double.parseDouble(alist.get(i).toString());
	...
	public static void normliseChangeRatio(ArrayList alist) {
		int k = -1;
		double maxRatio = Double.parseDouble(alist.get(findMaxStage(alist))
				.toString());
		double aRatio;
		for (int i = 1; i < (alist.size()); i++) {
	...
	public static ArrayList updateRatio_normalise_seem_problem_still(
			ArrayList alist) {
		...
		for (int i = 1; i < (alist.size() - 1); i++) {
			VarianceRatios.set(i,
					Double.parseDouble(VarianceRatios.get(i).toString())
							/ maxDiff);
		}
		VarianceRatios.add(-10000.0);
	...
	public static double[] setBestResult(double[] bestResults, int bestStage,
			ArrayList fmeasure, ArrayList Dunns, ArrayList Variances,
			ArrayList silhouette, ArrayList daviesDoublin) {

		if (bestStage == 1) {
			System.out.println("best stage=" + bestStage);
		}
		bestResults[0] = to4DecPlace(Double.parseDouble(Dunns.get(bestStage)
				.toString()));
		bestResults[1] = to4DecPlace(Double.parseDouble(fmeasure.get(bestStage)
				.toString()));
		// bestResults[2] = (double)(20-bestStage); // number of clusters
		bestResults[2] = (double) (Parameters.totalStage - bestStage); // number
																		// of
																		// clusters
		bestResults[3] = to4DecPlace(Double.parseDouble(silhouette.get(
				bestStage).toString()));
		bestResults[4] = to4DecPlace(Double.parseDouble(daviesDoublin.get(
				bestStage).toString()));
		return bestResults;
	}
	...
	public static double[][] updateRankTable(double[][] rankTable, int ref,
			double rank[]) {
		for (int i = 0; i < rank.length; i++) {
	...
	public static void silhouette_test(double[][] allClusters) {
		...
		for (int i = 0; i < allClusters.length; i++) {
			...
			for (int j = 0; j < allClusters[i].length; j++) {
				// compute a_i
				double a_i = averageDissimilarity(allClusters[i][j],
						allClusters[i]);
				...
				for (int p = 0; p < allClusters.length; p++) {
					...
					d_i_C = averageDissimilarity(allClusters[i][j],
							allClusters[p]);
	...
	public static double randindex(Vector bags[]) {
		...
		for (i = 0; i < n[s - 1].length; i++) {
			n_i = (double) n[s - 1][i];
			; // get the actual number in class i
			sum_n_i += nC2(n_i);
		}
		...
		for (i = 0; i < n[s - 1].length; i++) {
			...
			for (j = 0; j < bags.length; j++) {
				...
				while (iter.hasNext()) { // for each item in the cluster
					...
					// System.out.println("j = " + j + ", symbol = " +
					// dpTemp.getSymbol());
					if ((dpTemp.getSymbol() + "").compareTo(refSymbol) == 0)
						n_ij++;
				}
				sum_n_ij += nC2(n_ij);
			}
