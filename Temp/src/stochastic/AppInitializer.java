...
public class AppInitializer {
	...
	public void init_xCyD_datasets(int dim, int nClusters, String afilename) {
		...
		if (nClusters == 4) {
			...
			for (int i = 0; i < Parameters.clusterSizes.length; i++)
				Parameters.clusterSizes[i] = 0;
		} else {
	...
	public void initDataFromFile(String filename) { // data set from consensus
													// clustering
		...
		dataDetails = filehandle.readString(); // read the first line it is
												// dummy
		...
		if (aToken.contains("numClusters:")) {
			...
			System.out
					.println("Problem parsing num clusters. If you do not know the number of clusters, simply put 1.");
		}
		...
		if (aToken.contains("distType:")) {
			...
			System.out
					.println("Problem parsing distance type (1 for Cosine measure, 2 for Euclidean.");
		}
	...
	public void initWisconsin() {
		Parameters.dataSize = 683; // was 699, 683 is after removing missing
									// value
	...
	public void initDermatology() {
		Parameters.dataSize = 358; // was 699, 683 is after removing missing
									// value
	...
	/*****
	 * to delete public void initGUI(NewJFrame before, CustomCanvas
	 * beforeCanvas) { int frameOffsetX = 30; int frameOffsetY = 40 + 50;
	 * 
	 * before.setBackground(Color.BLUE); before.setSize(Parameters.maxX*5 +
	 * frameOffsetX, Parameters.maxY*5 + frameOffsetY);
	 * before.setTitle("Before Cluster");
	 * 
	 * before.getContentPane().add(beforeCanvas); before.setVisible(true); }
	 ***/
